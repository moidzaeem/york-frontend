...
const Page = ({ params }) => {
    ...
    const {isSubmitted, submitData} = useSubmitData('/api/categories/update');
    const router = useRouter();
    const {permissions, can} = useAccessControl();

    if (!can(["Categories - View", "Categories - Edit"])) {
        // navigate to 403 page.
        router.push(process.env.NEXT_PUBLIC_UNAUTHORIZED_ROUTE);
    }

    const { initialData } = useFetchResource("/api/categories", params.id);
    ...
    return (
        <>
            <title>YORK - Categories -- Edit</title>
            ...
            <Card className="h-auto min-h-96 md:rounded-[15px] bg-white md:bg-white px-0 md:px-4 dark:bg-transparent md:dark:bg-gray-800" >
                ...
                <div className="w-full px-4 mt-12">
                    <form onSubmit={ can("Categories - Edit") ? formik.handleSubmit : () => false }>
