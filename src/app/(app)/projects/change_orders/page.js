...
const Page = () => {
    ...
    if (!can(["ChangeOrders - View", "ChangeOrders - Create", "ChangeOrders - Edit", "ChangeOrders - Delete"])) {
    ...
    const handlePhotosPopup = (changeOrderID) => {
        setResourceID(changeOrderID);
    ...
    const handleCommentsPopup = (changeOrderID) => {
        setResourceID(changeOrderID);
    ...
    const handleDeletionPopup = (changeOrderID) => {
        setResourceID(changeOrderID);
    ...
    const handleArchivePopup = (changeOrderID) => {
        setResourceID(changeOrderID);
    ...
    const actionButtons = (
        <>
            {
                can("ChangeOrders - Create") ?
                    <BaseLink href="/projects/change_orders/new" className="bg-white md:bg-[#f5f5f5] text-base md:text-base" >
            ...
            <BaseButton onClick={async () => { await exportToExcel("ChangeOrders") }} className="bg-white md:bg-[#f5f5f5] text-base md:text-base" >
                <Download className="fill-slate-500" />
                Download
            </BaseButton>

           { can("Archive - Create")  ? <BaseLink href="/projects/change_orders/archive" className="bg-white md:bg-[#f5f5f5] text-base md:text-base" >
                    ...
                    return <ButtonGroupH>
...

                        {
                            can("ChangeOrders - Edit") ?
                                <SimpleIconLink href={`/projects/change_orders/${row.id}`} className="border border-gray-600" title="Edit">
                                    <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
    ...
    return (
        <>
            <title>YORK - Change Orders</title>
            ...
            <DeleteConfirmationModal
                ...
                remoteEndPoint='/api/change_order/delete'
            ...
            <ArchiveConfirmationModal
                ...
                resourceID={resourceID}
                remoteEndPoint='/api/change_order/archive'
            ...
            <CommentsModal
                ...
                commentsEndpoint="/api/change_order/comments"
                newCommentEndpoint="/api/change_order/comment"
                resourceID={resourceID}
                onClose={closeCommentsModal} />

            <PhotosModal
                isVisible={isPhotosModalVisible}
                photosEndpoint="/api/change_order/files"
                newPhotoEndpoint="/api/change_order/file"
                resourceID={resourceID}
